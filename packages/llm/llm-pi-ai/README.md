...
- **The live builtin catalog trusts `pi.dev` descriptors** — responses are bounded and validated before publication, but their model metadata becomes the next request snapshot. An explicit non-empty `models` list is the deployment-owned alternative for routes that must not follow remote additions or metadata changes.
