...
Deferred-tool compatibility is configurable per route or model: `deferredToolsMode: kimi` selects Chat Completions serialization; `supportsToolSearch` and `supportsAdditionalTools` select Responses serialization; `supportsToolReferences` selects Anthropic tool references. Responses fields also apply to Azure and Codex catalog routes. Model values override route values, which override catalog values; omitted fields retain pi-ai defaults. Boolean fields accept `false`; Kimi mode has no disabling value. When both Responses switches are true, `additional_tools` wins. The endpoint must support the selected format, and a context integration must supply pi-ai `addedToolNames`; these switches alone do not defer Harness tools or add a search tool. Request placement and cache-prefix reuse change only when that context exists.
