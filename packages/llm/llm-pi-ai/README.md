...
Generic multi-provider adapter for the harness LLM seam backed by [`@earendil-works/pi-ai`](https://www.npmjs.com/package/@earendil-works/pi-ai). One plugin instance owns a dict of provider profiles keyed by route; every request selects a profile with `GenerateOptions.provider` and resolves `GenerateOptions.model` against that route's configured catalog. A route naming an installed pi-ai provider inherits its endpoint, wire protocol, and live model catalog as defaults and overrides them field by field; a route pi-ai does not ship is declared outright, so an OpenAI-compatible gateway or self-hosted server is configuration rather than a code change.
...
- id: llm
  ...
  config:
    catalogRefreshIntervalMs: 300000
    providers:
...
A profile's non-empty `models` list selects the models an installed route serves from its current live catalog; omitting it (or leaving it empty) serves the whole catalog. Complete descriptors from `pi.dev` replace matching installed entries by id and may add models, while the installed catalog remains the offline baseline. Each selected entry inherits its current descriptor, including its wire protocol and endpoint, and fields beside `id` override that descriptor. Declaring a non-empty list means every model the route should keep serving must appear in it, an entry of nothing but `id` being enough. Hand-declared routes still define their catalog with `models`, route-level `api` and `baseURL`, and configured fallback capacities. The configurable entry fields are `id`, `name`, `contextWindow`, `maxTokens`, `input`, `reasoningEfforts`, and `compat`. Pricing has no harness consumer and rides the selected catalog entry or is absent.

`modelOverrides` reshapes individual live-catalog models without selecting a subset: each key is a catalog model id, each value the same fields a `models` entry takes with the id living in the key, and the rest of the catalog keeps serving untouched. Remote-only ids are valid after discovery has cached their descriptors. An override becomes that catalog entry's configuration, so capacities, efforts, and compat resolve through the same path with the same diagnostics and request-default semantics as a `models` entry. Overrides are only meaningful on an installed route serving its live catalog: one set beside a non-empty `models` list or on a hand-declared route is refused rather than skipped.
...
The adapter exposes each configured route's current immutable snapshot through `ctx.llm.listModels(provider)`. A successful catalog publication invalidates the memoized profiles, so the next list, resolve, or stream operation receives the new descriptors while an in-flight request keeps the snapshot it started with. `ctx.llm.resolveModelInfo(provider, model)` performs that exact descriptor lookup once and returns its identity, context window, configured output cap, and selectable thinking levels. A model's **configured** `maxTokens` becomes the seam's `defaultMaxTokens`, so a request that names no output cap carries the one the deployment chose; a value inherited from a catalog is the model's output *capability* and never becomes a request default on its own.
...
The adapter forces pi-ai's SDK `maxRetries` to zero so one `stream()` call makes one provider request. It also selects final tool-argument parsing: pi-ai forwards every raw argument delta unchanged and parses the complete JSON once when the tool call ends, which avoids reparsing the growing prefix for every chunk. The removed profile fields `maxRetries` and `maxRetryDelayMs` fail load instead of silently multiplying or hiding the separately composed agent-level retry budget. Idle expiry aborts the SDK's stable request signal and surfaces `TIMEOUT`; an earlier caller abort remains `ABORTED`.

## Endpoint interrogation

The plugin offers `ctx.llm.registerModelDiscovery('llm-pi-ai', …)`, which answers "which models can this provider serve?" for a route a configuration surface is editing or drafting. Builtin and hand-declared providers use different discovery sources.

A request naming a builtin provider forces and awaits `https://pi.dev/api/models/providers/<provider>`, bypassing the normal freshness TTL. It works while the adapter is dormant and does not resolve a request credential or require `baseURL`. A successful response persists complete descriptors and immediately supplies the discovery result; a network or validation failure returns `DISCOVERY_FAILED` while the last published catalog remains available to configured routes. Caller cancellation returns `ABORTED` without cancelling refresh work another caller is awaiting.

A hand-declared draft carries the credential the user typed, if any; a route that already stored one shows a configuration surface only a redacted descriptor, so the interrogation resolves that route's `apiKeyEnv` rather than going out unauthenticated and reporting the endpoint's 401 as a wrong key. A typed key wins, being the one under test. A supplied or stored probe key is trimmed and format-checked the same way, so a value no HTTP header can carry is refused immediately as `LlmError('INVALID_CREDENTIAL')` instead of reaching `fetch`, where it would surface as an opaque `ByteString` failure indistinguishable from an unreachable endpoint.
...
- **The live builtin catalog trusts `pi.dev` descriptors** — responses are bounded and validated before publication, but their model metadata becomes the next request snapshot. A non-empty `models` list narrows which current descriptors the route serves; it does not freeze their metadata. A deployment that must own every descriptor uses a hand-declared route with explicit route protocol and endpoint.
