...
Use the official MCP SDK v2 packages [`@modelcontextprotocol/client`](https://www.npmjs.com/package/@modelcontextprotocol/client), [`@modelcontextprotocol/core`](https://www.npmjs.com/package/@modelcontextprotocol/core), and the `@modelcontextprotocol/client/stdio` subpath (`Client`, `StdioClientTransport`, `StreamableHTTPClientTransport`). Each client generation sets `versionNegotiation: { mode: 'auto' }`, so the SDK probes modern `server/discover` and falls back to legacy `initialize` without a hardcoded protocol header. The harness does not implement its own JSON-RPC — consistent with how ACP delegates to `@agentclientprotocol/sdk`.
...
2. Configure the v2 client's list-change handler: legacy `notifications/tools/list_changed` and modern `subscriptions/listen` events both re-run the same sync (dispose previous generation, register new). Deterministic names mean unchanged tools keep their names across re-syncs.
...
- **E2E** (`tests/mcp-client.e2e.ts`, keyless): the real MCP v2 protocol against the in-repo `serveStdio` fixture, `@modelcontextprotocol/server-everything` for legacy fallback, and `@modelcontextprotocol/server-filesystem` over stdio, plus a strict modern `createMcpHandler` fixture over stateless Streamable HTTP — discovery under the namespace, dotted-name normalization end to end, execution round-trips, durable image save/read with base64 retained only in the canonical value, explicit refusal without an image route, duplicate-`serverName` rejection, and disposal.
...
- **MCP SDK stability**: v2 splits the client runtime from the raw wire schemas, and breaking API or protocol-era changes require updating the bridge. The lockfile pins the tested 2.0.0 package graph; release verification also exercises the built entry against an official Host.
