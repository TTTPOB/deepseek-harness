...
- `adapter.ts` 把每次解析变成一份**不可变快照**——profiles 加上持有这些 provider 的 `createModels()` 集合——每个操作都在自己第一个 `await` 之前整体捕获一份。它为每次 `streamSimple()` 请求选择 `toolCallParsing: 'final'`：pi-ai 保留每个原始参数 delta，并只在工具调用结束时解析一次完整 JSON，从而匹配 Harness 的原始 JSON 映射，不必为每个不断增长的前缀重复解析。
...
- **运行时动态 catalog**——`fetchModels` 加 `ModelsStore`，后台刷新。本次变更拒绝它，是因为产品需求仅为一次性发现动作，当时也没有完整远端描述符来源、缓存、失效机制或离线路径。后来的[内置实时 catalog 决定](2026-08-22-pi-ai-live-model-catalogs.zh.md)为已安装提供方补齐这些机制；手工声明路由保留本文决定的一次性 settings 路径。
